...
export class CompletionDataProvider {
  ...
  buildSelector(labels) {
    const allLabelTexts = labels.map(
      (label) =>
        `${label.name}${label.op}"${escapeLabelValueInExactSelector(
          label.value,
        )}"`,
    );
  ...
  async getParserAndLabelKeys(logQuery) {
    ...
    const cachedLabelKeys = this.queryToLabelKeysCache.has(logQuery)
      ? this.queryToLabelKeysCache.get(logQuery)
      : null;
    if (cachedLabelKeys) {
      ...
      if (
        this.queryToLabelKeysCache.size >= EXTRACTED_LABEL_KEYS_MAX_CACHE_SIZE
      ) {
      ...
      const labelKeys = await this.languageProvider.getParserAndLabelKeys(
        logQuery,
        { timeRange: this.timeRange },
      );

...
export function isQueryWithParser(query) {
  const nodes = getNodesFromQuery(query, [
    LabelParser,
    JsonExpressionParser,
    Logfmt,
  ]);
...
export function getLogQueryFromMetricsQuery(query) {
  ...
  const pipelineExpr = pipelineExprNode
    ? query.substring(pipelineExprNode.from, pipelineExprNode.to)
    : "";
...
export function getLogQueryFromMetricsQueryAtPosition(query, position) {
  ...
  return getLogQueryFromMetricsQuery(
    query.substring(metricQuery.from, metricQuery.to),
  );
}
...
function resolvePipeError(node, text, pos) {
  ...
  if (
    exprNode?.parent?.type.id === LogExpr ||
    exprNode?.parent?.type.id === LogRangeExpr
  ) {
...
function resolveLogfmtParser(_, text, cursorPosition) {
  ...
  const position =
    trimRightTextLen < cursorPosition ? trimRightTextLen : cursorPosition;

...
export const PIPE_PARSERS = [
  ...
  },
  {
    ...
    insertText: "pattern",
    documentation:
      "Extracting labels from the log line using pattern parser. Only available in Loki 2.3+.",
  },
  {
    label: "unpack",
    insertText: "unpack",
...
export const PIPE_OPERATORS = [
  {
    ...
    documentation:
      "Take labels and use the values as sample data for metric aggregations.",
  },
  {
    label: "label_format",
    insertText: "label_format",
    documentation:
      "Use to rename, modify or add labels. For example, | label_format foo=bar .",
  },
  {
    label: "line_format",
    insertText: "line_format",
    documentation:
      'Rewrites log line content. For example, | line_format "{{.query}} {{.duration}}" .',
  },
];
...
export const lokiGrammar = {
  ...
  "context-pipe": {
    ...
    inside: {
      ...
      "pipe-operations": {
        pattern: new RegExp(
          `${[...PIPE_PARSERS, ...PIPE_OPERATORS].map((f) => f.label).join("|")}`,
          "i",
        ),
        alias: "keyword",
      },
